...
function getAxisTitle(axis) {
  var axisType = (0, _lib.capitalize)(axis._name.split('axis')[0]);
  var subplotNumber = getAxisNumber(axis) || 1;
  return axis._input && axis._input.title ? (0, _lib.striptags)("".concat(axisType, ": ").concat(axis._input.title.text)) : (0, _lib.striptags)("".concat(axisType, " ").concat(subplotNumber));
}
